...
def seed():
    ...
    if db.query(User).count() == 0:
        db.add_all([
            User(username="admin", password=hash_password("admin123"), nickname="管理员", role="admin"),
            User(username="test", password=hash_password("test123"), nickname="测试用户", role="user"),
        ])
        db.commit()
        print("[OK] 用户数据已创建")
    else:
